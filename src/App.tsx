...
function App() {
  ...
  return (
    <div className="mx-auto flex min-h-screen max-w-7xl flex-col lg:flex-row">
      <header
        className="sticky top-0 z-20 flex items-center justify-between border-b border-slate-200 bg-white/95 px-4 py-3 backdrop-blur lg:hidden dark:border-slate-700 dark:bg-slate-900/95"
        style={{ paddingTop: 'max(0.75rem, env(safe-area-inset-top))' }}
      >
        <span className="text-base font-semibold">EEE Toolkit</span>
        <button
          onClick={() => setNavOpen((v) => !v)}
          className="rounded-md border border-slate-300 px-3 py-2 text-sm active:bg-slate-100 dark:border-slate-600 dark:active:bg-slate-800"
        >
          {navOpen ? 'Close' : 'Menu'}
        </button>
      </header>
